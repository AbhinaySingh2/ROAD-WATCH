...
export default function ReportPage() {
    ...
    const syncOffline = async () => {
      ...
      for (const r of queue) {
        try {
          ...
          fd.append("latitude", r.latitude.toString()); fd.append("longitude", r.longitude.toString());
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(p => (setLocation({ lat: p.coords.latitude, lng: p.coords.longitude }), setLocationLoading(false)), () => (setSubmitError("Enable GPS."), setLocationLoading(false)), { enableHighAccuracy: true });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!navigator.onLine) {
      ...
      q.push({ latitude: location.lat, longitude: location.lng, description, imageSrc, filename: file.name, mimeType: file.type, timestamp: Date.now() });
      localStorage.setItem("offline_reports", JSON.stringify(q));
      setReportId(null); setImpactScore(50); setSuccess(true); return;
    }
    ...
    try {
      const [data] = await Promise.all([apiFetch<any>("/api/v1/issues/submit", { method: "POST", body: fd }), new Promise(r => setTimeout(r, 600))]);
    ...
    return (
      <div className="min-h-screen bg-[#FAF9F5] text-stone-900 flex items-center justify-center p-6 relative font-sans">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="z-10 w-full max-w-lg bg-white border border-black/5 p-8 rounded-[36px] shadow-sm flex flex-col items-center">
          ...
          <h1 className="text-3xl font-black mb-2 uppercase">{reportId === null ? "Report Queued!" : "Report Logged!"}</h1>
          <p className="text-sm text-stone-500 text-center max-w-sm mb-8 font-semibold">{reportId === null ? "You are currently offline. Report saved locally and will auto-sync when network returns." : "Gemini AI triaged the photo and mapped the coordinates."}</p>
          <div className="grid grid-cols-2 gap-4 w-full mb-8">
            {[["Report ID", reportId === null ? "OFFLINE-QUEUE" : `#${reportId}`, "text-lg font-mono font-extrabold text-stone-850", ""], ["Status", reportId === null ? "QUEUED" : "PENDING", "", "", true], ["Coordinates", location ? `${location.lat.toFixed(5)}, ${location.lng.toFixed(5)}` : "N/A", "text-[10px] font-mono text-stone-700", ""], ["Impact Points", `+${impactScore}`, "text-xl font-black text-[#FF5A1F]", "bg-orange-500/5 border-orange-500/20"]].map(([lbl, val, cls, col, badge]: any, idx) => (
              <div key={idx} className={`border rounded-2xl p-4 text-center flex flex-col justify-center min-h-[92px] shadow-sm ${col || 'bg-stone-50 border-black/5'}`}><p className="text-[10px] text-stone-400 mb-1.5 font-bold uppercase tracking-widest">{lbl}</p>{badge ? <div className="inline-block mx-auto px-2.5 py-0.5 bg-orange-50 border border-orange-200 text-[#FF5A1F] text-[9px] font-black tracking-widest rounded-full">{reportId === null ? "QUEUED" : "PENDING"}</div> : <p className={cls}>{val}</p>}</div>
            ))}
          </div>
      ...
      <main className="max-w-xl mx-auto p-6 w-full flex-1 flex flex-col justify-center z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-black/5 p-8 rounded-[36px] shadow-sm relative overflow-hidden">
          <AnimatePresence>
            {isSubmitting && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-white/95 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center">
                ...
                {imageSrc && <div className="relative w-56 aspect-video bg-stone-50 border border-orange-500/20 rounded-2xl overflow-hidden shadow-inner mb-6 mx-auto"><img src={imageSrc} alt="Scanning" className="w-full h-full object-cover opacity-80" /><div className="absolute left-0 right-0 h-1 bg-[#FF5A1F] scanning-line" /></div>}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2 text-left">
              ...
              {imageSrc ? (
                <div className="relative rounded-2xl overflow-hidden bg-stone-50 border border-orange-500/20 aspect-video flex items-center justify-center shadow-inner group">
                  <img src={imageSrc} alt="Pothole Damage Scan" className="w-full h-full object-cover" /><div className="absolute inset-x-0 h-0.5 bg-[#FF5A1F] animate-bounce" style={{ animationDuration: '3s' }} />
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><button type="button" onClick={() => fileInputRef.current?.click()} className="bg-[#FF5A1F] hover:bg-[#E84E15] text-white font-bold text-xs uppercase tracking-widest py-2.5 px-6 rounded-full cursor-pointer shadow-md">Reselect Photo</button></div>
                </div>
              ) : (
                <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full aspect-video border border-dashed border-stone-200 hover:border-orange-500/40 hover:bg-orange-500/[0.01] rounded-2xl flex flex-col items-center justify-center text-stone-400 transition-all cursor-pointer group p-6 shadow-inner relative"><div className="w-14 h-14 bg-stone-50 border border-black/5 rounded-2xl flex items-center justify-center mb-4"><Camera className="w-5 h-5 text-[#FF5A1F]" /></div><span className="font-extrabold text-xs uppercase tracking-wider text-stone-700">UPLOAD PHOTO / TAKE PICTURE</span></button>
              )}
            </div>
            ...
            <div className="space-y-2 text-left">
              ...
              <div className="p-4 bg-stone-50 border border-black/5 rounded-2xl flex items-center justify-between shadow-inner">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-xs font-extrabold uppercase">Coordinates</p><p className="text-[11px] text-stone-400 mt-1 font-mono">{location ? `${location.lat.toFixed(6)}, ${location.lng.toFixed(6)}` : "ACQUIRING GPS..."}</p></div>
                </div>
                <button type="button" onClick={getLocation} disabled={locationLoading} className="px-4 py-2.5 bg-white border border-black/5 text-stone-750 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 cursor-pointer shadow-sm">
                  {locationLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin text-[#FF5A1F]" /> : null}{location ? "Refresh GPS" : "Acquire GPS"}
                </button>
              </div>
            </div>
            ...
            <button type="submit" disabled={!file || !location || isSubmitting} className="w-full py-4.5 bg-[#FF5A1F] hover:bg-[#E84E15] disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-full font-bold text-xs uppercase tracking-widest transition-all cursor-pointer">{isSubmitting ? "SUBMITTING..." : !location ? "ACQUIRING GPS..." : !file ? "ADD IMAGE TO SUBMIT" : "SUBMIT REPORT"}</button>
          </form>
        </motion.div>
      </main>
    </div>
