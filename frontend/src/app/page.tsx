...
export default function Home() {
  ...
  useEffect(() => {
    if (!data || data.length === 0) { setHealthIndex(data ? 100 : 84); setActiveReports(data ? 0 : 14); return; }
    setActiveReports(data.length);
    const crit = data.filter((r: any) => r.severity === 'CRITICAL' && r.status !== 'RESOLVED').length, high = data.filter((r: any) => r.severity === 'HIGH' && r.status !== 'RESOLVED').length;
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 pt-16 sm:pt-28 pb-12 flex flex-col justify-center z-10 space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={idx} className="border border-black/5 bg-white hover:border-orange-500/20 shadow-sm p-8 rounded-[32px] transition-all text-left relative group"><div className="absolute top-0 right-0 w-20 h-20 bg-orange-500/[0.01] blur-2xl rounded-full" /><div className="w-12 h-12 bg-stone-50 rounded-2xl border border-black/5 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform"><Icon className="w-5.5 h-5.5 text-[#FF5A1F]" /></div><h3 className="text-sm font-extrabold uppercase tracking-wider text-stone-900 mb-2">{title}</h3><p className="text-stone-500 text-xs font-semibold leading-relaxed">{desc}</p></div>
          ))}
        </div>
      </main>
    </div>
