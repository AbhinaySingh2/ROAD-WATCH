...
export default function ClaimGenerator() {
  ...
  const handleGenerate = (e: React.FormEvent) => {
    ...
    setIsAnalyzing(true);
    const logs = ["AI Scanning vehicle damage photo...", "Analyzing structural rim & tire integrity...", "Cross-referencing pothole database...", "Matching incident location data...", "Citing municipal public liability codes...", "Generating official compensation challan claim..."];
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-7 flex flex-col">
      <div className="bg-white border border-black/5 p-8 rounded-[36px] flex flex-col justify-between h-full min-h-[460px] shadow-sm relative text-left">
        ...
        <AnimatePresence mode="wait">
          {isAnalyzing ? (
            <motion.div key="scan" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 flex flex-col items-center justify-center p-6 text-center" >
            ...
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col justify-between space-y-5" >
              ...
              <div className="flex gap-4">
                <button onClick={() => setClaimLetter(null)} className="w-full py-4.5 bg-stone-100 hover:bg-stone-200 border border-black/5 text-stone-750 font-bold text-xs uppercase tracking-widest rounded-full active:scale-95 transition-all text-center">
                  Back
                </button>
                <button onClick={handlePrint} className="w-full py-4.5 bg-[#FF5A1F] hover:bg-[#E84E15] text-white font-bold text-xs uppercase tracking-widest rounded-full active:scale-95 transition-all flex items-center justify-center gap-2 shadow-lg">
                  <Printer className="w-4 h-4" /> Print / Save PDF
                </button>
              </div>
            </motion.div>
          ) : (
            <motion.form key="form" onSubmit={handleGenerate} className="flex-1 flex flex-col justify-between space-y-5" >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="block text-[9px] uppercase font-bold text-stone-400 tracking-widest">1. VEHICLE DAMAGE PHOTO</label>
                  ...
                  {imageSrc ? (
                    <div className="relative aspect-video rounded-2xl overflow-hidden bg-stone-50 border border-orange-500/20 shadow-inner group flex items-center justify-center">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button type="button" onClick={() => fileInputRef.current?.click()} className="bg-[#FF5A1F] hover:bg-[#E84E15] text-white font-bold text-[9px] uppercase tracking-widest py-2 px-4 rounded-full shadow-md">Change Photo</button>
                      </div>
                    </div>
                  ) : (
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full aspect-video border border-dashed border-stone-200 hover:border-orange-500/40 hover:bg-orange-500/[0.01] rounded-2xl flex flex-col items-center justify-center text-stone-400 transition-all cursor-pointer p-4 shadow-inner" >
                      <Camera className="w-5 h-5 text-[#FF5A1F] mb-2" />
                      <span className="font-extrabold text-[9px] uppercase tracking-wider text-stone-700">Snap Damage Photo</span>
                    </button>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="block text-[9px] uppercase font-bold text-stone-400 tracking-widest">2. ESTIMATED REPAIR BILL (₹)</label>
                    <input type="number" value={repairCost} onChange={(e) => setRepairCost(e.target.value)} placeholder="e.g. 15000" className="w-full bg-stone-50 border border-black/5 rounded-2xl px-4 py-3.5 text-xs font-black placeholder:text-stone-300 focus:outline-none" />
                  </div>

                  <div className="space-y-2">
                    <label className="block text-[9px] uppercase font-bold text-stone-400 tracking-widest">3. CROSS-REF INCIDENT</label>
                    <select value={selectedReportId} onChange={(e) => setSelectedReportId(e.target.value)} className="w-full bg-stone-50 border border-black/5 rounded-2xl px-4 py-3.5 text-xs font-black uppercase tracking-wider focus:outline-none">
              ...
              <div className="p-4 bg-orange-500/5 border border-orange-500/10 rounded-2xl flex items-start gap-2.5">
                ...
                <p className="text-[9px] font-semibold text-stone-500 leading-relaxed uppercase">
                  Under road liability charters, authorities are legally liable for damages if reported hazards are left unmaintained. This AI claims generator compiles the telemetry for court/insurance submittal.
                </p>
              </div>

              <button type="submit" disabled={!imageSrc || !selectedReportId} className="w-full py-4.5 bg-[#FF5A1F] hover:bg-[#E84E15] disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-full font-bold text-xs uppercase tracking-widest transition-all cursor-pointer shadow-lg active:scale-95">
