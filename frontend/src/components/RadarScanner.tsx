...
export default function RadarScanner() {
  ...
  const radarReports = (data || []).map((r: any) => {
    const dx = (r.longitude - userCoords.lng) * 111320 * Math.cos((userCoords.lat * Math.PI) / 180),
      dy = (r.latitude - userCoords.lat) * 111320;
    const colors: Record<string, string> = {
      CRITICAL: "bg-[#FF5A1F] shadow-orange-500/40",
      HIGH: "bg-amber-500 shadow-amber-500/40",
      MEDIUM: "bg-indigo-500 shadow-indigo-500/40",
    };
    return {
      id: `REPORT-${r.id}`,
      road: r.road_name || r.address || `${r.latitude.toFixed(4)}, ${r.longitude.toFixed(4)}`,
      severity: r.severity || "LOW",
      depth: r.infra_type || "OTHER",
      distance: Math.sqrt(dx * dx + dy * dy),
      angle: Math.atan2(dx, dy),
      coords: `${r.latitude.toFixed(5)}, ${r.longitude.toFixed(5)}`,
      color: colors[r.severity] || "bg-emerald-500 shadow-emerald-500/40",
    };
  });
  ...
  useEffect(() => {
    if (displayReports.length > 0 && !selectedTarget) setSelectedTarget(displayReports[0]);
  }, [displayReports, selectedTarget]);
  ...
  return (
    ...
    >
      <Card className="p-8 flex flex-col justify-between h-full min-h-[460px] relative overflow-hidden text-center group">
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center min-h-[264px]">
            <Loader2 className="w-8 h-8 animate-spin text-[#FF5A1F]" />
          </div>
        ) : displayReports.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center min-h-[264px] text-stone-400 font-bold uppercase text-xs">
            No active hazards detected in range
          </div>
        ) : (
          <div className="relative w-64 h-64 mx-auto my-6 flex items-center justify-center">
            ...
            {[
              "w-56 h-56 border-stone-200/50",
              "w-44 h-44 border-stone-200/30",
              "w-32 h-32 border-stone-200/50 border-dashed",
            ].map((cls, i) => (
              <div key={i} className={`absolute rounded-full border ${cls}`} />
            ))}
            <div
          ...
          {selectedTarget && (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${selectedTarget.severity === "CRITICAL" ? "bg-rose-500/10 border-rose-500/20 text-rose-600" : selectedTarget.severity === "HIGH" ? "bg-amber-500/10 border-amber-500/20 text-amber-600" : "bg-indigo-500/10 border-indigo-500/20 text-indigo-600"}`}
                >
              ...
              <div className="grid grid-cols-2 gap-2.5 text-[10px] text-stone-500 font-bold">
                <div>
                  ROADWAY:{" "}
                  <span className="text-stone-900 block mt-0.5 font-extrabold line-clamp-1">{selectedTarget.road}</span>
                </div>
                <div>
                  HAZARD / DIST:{" "}
                  <span className="text-stone-900 block mt-0.5 font-extrabold">
                    {selectedTarget.depth} ({Math.round(selectedTarget.distance)}m)
                  </span>
                </div>
              </div>
            </motion.div>
