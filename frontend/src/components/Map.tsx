...
const sevColor = (s: string) =>
  s === "CRITICAL"
    ...
    : s === "HIGH"
      ? "bg-orange-500/20 text-orange-450"
      : s === "MEDIUM"
        ? "bg-yellow-500/20 text-yellow-450"
        : "bg-green-500/20 text-green-450";
...
export default function Map({
  ...
  const heatReports = sorted.filter(
    (r) => r.damage_type === "POTHOLE" || r.severity === "CRITICAL" || r.severity === "HIGH",
  );
          ...
          <Popup className="custom-popup">
            <div className="p-1.5 min-w-[210px] text-white text-left font-sans">
              <h3 className="font-extrabold mb-1.5 text-sm text-transparent bg-clip-text bg-gradient-to-r from-white to-neutral-300">
                #{report.id} {report.infra_type || "Incident"}
              </h3>
              {report.image_url && (
                  ...
                  <img
                    src={
                      report.image_url.startsWith("http")
                        ? report.image_url
                        : `${(process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000").replace(/\/+$/, "")}/${report.image_url}`
                    }
                    alt="Hazard preview"
              ...
              <p className="text-[11px] text-neutral-400 mb-2.5 flex items-center gap-1.5">
                Severity:
                <span className={`font-black px-2 py-0.5 rounded text-[10px] ${sevColor(report.severity)}`}>
                  {report.severity || "Unknown"}
                </span>
              </p>

              <div className="text-[11px] bg-white/5 border border-white/5 p-2 rounded-xl mb-2 text-neutral-300">
                <strong className="text-neutral-400">Assigned:</strong>{" "}
                {report.assigned_authority || "Pending Assessment"}
              </div>
              ...
              <div className="flex items-center justify-between mt-3.5 pt-2.5 border-t border-white/10">
                ...
                {onUpvote && (
                  <button
                    ...
                    className="bg-gradient-to-r from-orange-500 to-amber-500 hover:opacity-90 text-white font-extrabold text-[10px] py-1.5 px-3 rounded-full shadow-md cursor-pointer active:scale-95 transition-transform"
                  >
                    Upvote
                  </button>
                )}
