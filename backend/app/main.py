...
class Report(Base):
    ...
    assigned_authority = Column(Enum(Authority), default=Authority.UNKNOWN, index=True)
...
class AISettingsUpdate(BaseModel):
    ...
    scan_density: Optional[str] = None
...
async def reverse_geocode(lat: float, lon: float) -> Tuple[Optional[str], Optional[str]]:
    try:
        ...
        if loc:
            return loc.address, loc.raw.get("address", {}).get("road") or loc.raw.get("address", {}).get("highway")
    except Exception as e:
...
def determine_authority(road_name: str) -> str:
    ...
    return (
        ...
        else "STATE_HIGHWAY" if "SH" in n or "STATE HIGHWAY" in n else "MUNICIPAL"
    )
...
origins = (
    [o.strip() for o in settings.CORS_ALLOW_ORIGINS.split(",") if o.strip()] if settings.CORS_ALLOW_ORIGINS else ["*"]
)
app.add_middleware(
    CORSMiddleware,
...
@app.post(
    f"{settings.API_V1_STR}/issues/submit",
...
async def submit_report(
    ...
    if image:
        ...
        if ext not in ["jpg", "jpeg", "png", "webp"]:
            raise HTTPException(
                status_code=400,
                detail="Invalid file type. Only JPG, JPEG, PNG, and WEBP allowed.",
            )
        ...
        try:
            ...
            existing_report = (
                await db.execute(select(Report).where(Report.image_hash == image_hash))
            ).scalar_one_or_none()
            if existing_report:
                raise HTTPException(
                    status_code=409,
                    detail="Duplicate rejected: This exact image has already been reported.",
                )
            # ---------------------------------------------------
    ...
    if ai_res and not ai_res.is_road_damage:
        fail(f"AI Validation Failed: {ai_res.ai_description}")
    if ai_res and ai_res.ai_confidence < (ai_settings.get("min_confidence", 75.0) / 100.0):
        fail(
            f"AI Triage Failed: Confidence level ({int(ai_res.ai_confidence * 100)}%) is below the required threshold ({int(ai_settings['min_confidence'])}%)."
        )

    # --- Added image_hash to the database insertion ---
    data = {
        "latitude": latitude,
        "longitude": longitude,
            ...
            (
                {
                    ...
                    "infra_type": ai_res.infra_type,
                    "ai_confidence": ai_res.ai_confidence,
                    "ai_description": ai_res.ai_description,
                    "impact_score": ai_res.impact_score,
...
async def get_reports(
    ...
    col = getattr(Report, sort_by, Report.id)
    query = select(Report).order_by(col.desc() if sort_order == "desc" else col.asc())
...
async def update_report(id: int, r_update: ReportUpdate, db: AsyncSession = Depends(get_db)):
    ...
    for k, v in r_update.model_dump(exclude_unset=True).items():
        setattr(rep, k, v)
    await db.commit()
    await db.refresh(rep)
    return rep
...
async def upvote_report(id: int, db: AsyncSession = Depends(get_db)):
    ...
    if rep.upvotes > 0 and rep.upvotes % 10 == 0:
        rep.impact_score = min(100, (rep.impact_score or 0) + max(1, int((rep.impact_score or 0) * 0.05)))
    await db.commit()
    await db.refresh(rep)
    return rep
...
async def generate_challan(id: int, db: AsyncSession = Depends(get_db)):
    ...
    items = [
        ...
        (
            "Severity Level",
            str(rep.severity.value if hasattr(rep.severity, "value") else rep.severity),
        ),
        (
            "Hazard Type",
            str(rep.infra_type.value if hasattr(rep.infra_type, "value") else rep.infra_type),
        ),
        (
            "Assigned Authority",
            str(rep.assigned_authority.value if hasattr(rep.assigned_authority, "value") else rep.assigned_authority),
        ),
    ...
    grid_html = "\n".join(
        f'<div class="text-[11px] font-bold uppercase text-[#8F8D88]">{lbl}<span class="block text-sm font-black text-[#1F1E1B] mt-1 font-mono">{val}</span></div>'
